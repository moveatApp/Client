...
export default function WeightChart() {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex justify-between items-center mt-4 px-4 bg-gray-50 dark:bg-white/5 rounded-full py-2 max-w-md mx-auto w-full text-xs font-bold text-gray-500">
        <button 
          onClick={() => setPeriod('1d')}
          className={`px-4 py-1.5 rounded-full transition-colors ${period === '1d' ? 'bg-gray-300/40 dark:bg-white/10 text-foreground' : ''}`}>1 Día</button>
        <button 
          onClick={() => setPeriod('1w')}
          className={`px-4 py-1.5 rounded-full transition-colors ${period === '1w' ? 'bg-gray-300/40 dark:bg-white/10 text-foreground' : ''}`}>1 Sem</button>
        <button 
          onClick={() => setPeriod('1m')}
          className={`px-4 py-1.5 rounded-full transition-colors ${period === '1m' ? 'bg-gray-300/40 dark:bg-white/10 text-foreground' : ''}`}>1 Mes</button>
        <button 
          onClick={() => setPeriod('1y')}
          className={`px-4 py-1.5 rounded-full transition-colors ${period === '1y' ? 'bg-gray-300/40 dark:bg-white/10 text-foreground' : ''}`}>1 Año</button>
      </div>
    </div>
  );
}
